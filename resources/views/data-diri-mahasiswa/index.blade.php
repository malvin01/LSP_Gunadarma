    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('mahasiswa.store') }}" method="POST">
              ...
              <form action="{{ route('data-diri-mahasiswa.uploadKrs') }}" enctype="multipart/form-data" method="POST">
                @csrf
              <div class="shadow overflow-hidden sm:rounded-md mt-5">
                <div class="px-4 py-5 bg-white sm:p-6">
                  <div class="grid grid-cols-6 gap-6">
                    @if (auth()->user()->getFirstMediaUrl('krs'))
                      <a href="{{ route('data-diri-mahasiswa.downloadFile') }}">Donwload KRS</a>
                    @else
                      <div class="col-span-6">
                        <label for="krs" class="block text-sm font-medium text-gray-700
                        {{ $errors->has('krs') ? 'text-red-600' : '' }}">KRS</label>
                        <input type="file" name="krs" id="krs" 
                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md
                        {{ $errors->has('krs') ? 'bg-red-50 border border-red-500 text-red-900 focus:ring-red-500 focus:border-red-500' : '' }}">
                        @error('krs')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                      </div>
